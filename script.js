...
function toggleModal(event) { //открываю модальное окно
...
const nameInput = document.querySelector( // Находим поля формы в DOM
...
const initialCards = [ //работа с добавлением карточек задание №2
  {
    name: "Любовь",
...
window.onload = function(){ //выполнение скрипта после загрузки, помогает избежать ошибку в консоли
  initialCards.forEach(function (element) {
    ...
    initialCardElement.querySelector('.element__title-text').textContent = element.name;// Укажите здесь значение поля career каждого перебираемого элемента;
  ...
  elementImage.forEach(function(photo){
    photo.addEventListener("click", function(picture){
      ...
      const elementText = currentElement.querySelector('.element__title-text').textContent //ищем подпись в ближайшем определенном блоке, который раньше выбрали
...
const CardImage = document.querySelector( //находим поля формы в DOM
...
function handleFormAdd(evt) {
  ...
  CardElement.querySelector('.element__title-text').textContent = CardImageValue;
